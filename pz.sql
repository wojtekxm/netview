...
CREATE TABLE device (
	...
	PRIMARY KEY (id),
	UNIQUE INDEX unique_name (name),
	FOREIGN KEY key_controller (controller_id) REFERENCES controller (id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE device_survey (
	...
	PRIMARY KEY (id),
	FOREIGN KEY key_device (device_id) REFERENCES device (id) ON UPDATE CASCADE ON DELETE CASCADE
);
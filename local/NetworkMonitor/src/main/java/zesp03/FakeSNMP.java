...
public class FakeSNMP implements SNMPHandler {
    ...
    private void parse(String resourceName) throws IOException {
        ...
        try {
            ...
            if(stream == null)throw new IllegalStateException("resource \"" + resourceName + "\" for FakeSNMP is not available");
            try(InputStreamReader isr = new InputStreamReader(stream, "utf-8");
                BufferedReader reader = new BufferedReader(isr) ) {
                String controllerIP = reader.readLine();
                if(controllerIP == null)throw new IllegalStateException("invalid resource for FakeSNMP, IP is missing");
    ...
    @Override
    public List<DeviceInfo> queryDevices(String controllerIP) throws SNMPException {
        ...
        final ArrayList<DeviceInfo> result = new ArrayList<>();
        for(String name : names) {
            DeviceInfo info = new DeviceInfo();
            info.setName(name);
            // 80% szansy że będzie włączony
            info.setEnabled( random.nextInt(5) < 4 );
            // losowa liczba klientów z przedziału 0 .. 49
            info.setClients( random.nextInt(50) );
            result.add(info);
        }
        return result;
    }
